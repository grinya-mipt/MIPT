...
//PART 1: checking if the input is correct
		...
		// dup2(old,new) закрывает дескриптор new и копирует затем дескриптор old в new. Возможны исключительные ситуации:
		// 1. Если old дескриптора не существует, то вызов возвращает -1 и ставит errno определенное значение (см. man). Дескриптор new не закрывается.
		// 2. Если old корректен и old == new, то dup2() ничего не делает и возвращает значение дескриптора new
                if (dup2(fdp[1],1) == -1) {
			print_into_stderr("Error in dup2(). Invalid fdp[1] descriptor\n");
                        exit(-1);
			}
                if (close(fdp[0]) < 0) {
...
// Я когда пишу функции, руководствуюсь принципом: "чем меньше моя функция специфического, связанного с конкретно этой программой, делает, тем лучше". Лень потом о чем-то думать, постоянно что-то помнить, когда буду копировать эту функцию в другую программу. 
int print_into_stderr(const char str[])
{
...
int read_string_from_pipe(int fd_0,struct string* res,int buff_size)
{
        ...
        do
                ...
                if((size = read(fd_0,buffer,buff_size))<0)
                        ...
                        else res->str = strncat(res->str,buffer,size);
        		}
                else flag=0;
                }
        while(flag==1);        

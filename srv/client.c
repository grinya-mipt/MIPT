...
#define IPC_FTOK_FILE_SEND "/bin/ls"
#define IPC_FTOK_FILE_RECIEVE "/bin/cat"
...
//create or get semafors for logging
	key_t sem_key = ftok(IPC_FTOK_FILE_SEND, 2);
...
//send hello
	...
	msg_hello.data[0] = (int)pid;
	if(msgsnd(msgid_s, (void*) &msg_hello, sizeof(int), 0) < 0) print_err("send hello message");
	write_log(semid, fd_log, "Sent hello message to server");

//recieve work_size and decide "to work or not to work"
	struct client_msg msg_work_size, msg_bye;
	msg_work_size.mtype = (int)pid;
	if(msgrcv(msgid_r, (void*) &msg_work_size, sizeof(int), (int)pid, 0) < 0) print_err("get work_size message");
	if(msg_work_size.data[0] == 0){
		write_log(semid,fd_log,"Nothing to work with");
	//send BYE messege
		msg_bye.mtype = BYE_MSG_ID;
		msg_bye.data[0] = (int)pid;
		if(msgsnd(msgid_s, (void*) &msg_bye, sizeof(int), 0) < 0) print_err("send BYE to server");
		write_log(semid,fd_log,"Sent BYE message to server");		
	}else{
		int work_size = msg_work_size.data[0];
		write_log(semid,fd_log,"Recieved size of work");
	//recieve information for calculating
		struct client_msg msg_info;
		msg_info.mtype = (int)pid;
		if(msgrcv(msgid_r, (void*) &msg_info, 5*sizeof(int), (int)pid, 0) < 0) print_err("get additional information");
	...
	//recieve data for calculating    
		int* data = recieve_data(msgid_r, (int)pid, size_of_mtx_str, col_count, row_count);
	...
	//send result
		...
		for(i = 0; i < parts_number; i++){
			...
			if(msgsnd(msgid_s, (void*) &msg_result, size*sizeof(int), 0) < 0) print_err("send data to server");
	        	sent += size;
		}
...
//delete semaphores
	...
	msg_sem.mtype = (int)pid;
	write_log(semid,fd_log,"Ready for recieve BYE message from server and end work");
	if(msgrcv(msgid_r, (void*) &msg_sem, sizeof(int), (int)pid, 0) < 0) print_err("recieve BYE message from server");	
	if(msg_sem.data[0] == 0){
		write_log(semid,fd_log,"Semaphors for log is going to be deleted");
		write_log(semid,fd_log,"End work");
		msg_bye.mtype = (int)pid;
		if(msgsnd(msgid_s, (void*) &msg_bye, 0, 0) < 0) print_err("send confirmation to server");
